...
public class AlarmInitReceiver extends BroadcastReceiver {
	...
	private static final int ALARM_THRESHOLD = 2*60*1000; // 2min wangdanyang modify 20110715
        ...
        AsyncHandler.post(new Runnable() {
            @Override public void run() {
                ...
                if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
        			...
        			if (reason == true) {
						Log.v("alarm start!!!!!!");
                        Log.v("alarm savedAlarmTime!!!!!! = " +savedAlarmId );
                        Log.v("alarm savedAlarmTime!!!!!! = " +savedAlarmTime );
        				// Fire power-off alarm, and return to avoid to call
						// setNextAlert() again
						Log.v("savedAlarmId > 0 = " +(savedAlarmId > 0) );
                        Log.v("now - savedAlarmTime <=ALARM_THRESHOLD = " +(now - savedAlarmTime <=ALARM_THRESHOLD) );
